...
for i in range (len(l)):
    for j in range (len(l_p)):
        C_l[i,j]=np.exp(-((l[i]-l_p[j])**2.0)/(2.0*sigma_p**2.0))+10.0**(-5)
```

```{python}
moy_C_l = np.mean(C_l)
ecart_C_l = np.sqrt(np.var(C_l))
```
...
Ntrain = 10000-100
Ntest = 100
Max_C_l=np.abs(C_l).max()
NNC_l = (C_l-moy_C_l)/(np.abs(C_l).max())
X_train = Maps[:,0:(Ntrain)]
y_train = NNC_l[:,0:(Ntrain)]
X_test = Maps[:,(Ntrain):(Ntrain+Ntest)]
y_test = NNC_l[:,(Ntrain):(Ntrain+Ntest)]
y_train = y_train.T
y_test = y_test.T
print(y_train.shape, y_test.shape)
print(np.abs(NNC_l).max())
```
...
out=kr.layers.Activation('sigmoid')(x)
out_reshape=kr.layers.Reshape()(out)
```

### Model

```{python}
model = kr.models.Model(inputs=inputs, outputs=out_reshape)
model.compile(loss=kr.losses.mse, optimizer='adam', metrics=['accuracy', kr.metrics.mean_absolute_percentage_error])
model.summary()
```
...
```{python}
prediction = model.predict(X_test)*np.abs(C_l).max()+moy_C_l
```
